...
#-- Main
{
	...
	# key/value pairs generated by cmake for the Dyninst build
	my $cmake_cache;
...
sub setup_boost {
	my ($args, $build_dir, $cmake_cache) = @_;
	
	my $boost_inc = $cmake_cache->{'Boost_INCLUDE_DIR'};
	
	# Set up the Boost environment
	if ($boost_inc =~ /NOTFOUND/) {
		...
		$args->{'boost-inc'} = $boost_inc;
		$args->{'boost-lib'} = dirname($cmake_cache->{'Boost_SYSTEM_LIBRARY_RELEASE'});
	}
}
