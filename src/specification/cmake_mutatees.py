...
def print_one_cmakefile(exe, abi, stat_dyn, pic, opt, module, path, mlist, platform, cmakelists, info, directory):
    ...
    if lang == 'fortran':
        ...
        # This is kinda ugly, but we need to force usage of a particular
        # compiler. So
    ...
    out.write(
        "include_directories(\"${PROJECT_SOURCE_DIR}/testsuite/src/%s\")\n" % module)
    ...
    link_flags = "%s %s %s" % (compiler['flags']['link'],
                               compiler['abiflags'][platform['name']][
                                   mut['abi']]['flags'],
                               linkage)
    ...
    out.write(
        "include (${PROJECT_SOURCE_DIR}/%s/srclists.cmake)\n" % platform['name'])
    ...
    for m in mlist:
        ...
        out.write(
            "\t\tset_source_files_properties(${f} PROPERTIES LANGUAGE %s)\n" % lang)
        out.write("\tendif()\n")
        ...
        if len(m['libraries']) > 0:
            out.write("target_link_libraries (%s" %
                      utils.mutatee_binary(m, platform, info))
    ...
    for m in mutatees:
        ...
        if (is_groupable(m, info) == '1'):
            collected_srcs.append(
                '\t${PROJECT_SOURCE_DIR}/%s/%s_group.c\n' % (platform['name'], m['name']))
...
def print_compiler_cmakefiles(mutatees, platform, info, cmakelists, directory):
    ...
    for mut in mutatees:
        ...
        cmake_tree[exe][abi][stat_dyn][pic][opt][
            module].setdefault('mutatees', []).append(mut)
                ...
                for pic, tmp4 in tmp3.iteritems():
                    for opt, tmp5 in tmp4.iteritems():
                        for module, tmp6 in tmp5.iteritems():
                            ...
                            mlist = tmp6['mutatees']
                            print_one_cmakefile(
                                exe, abi, stat_dyn, pic, opt, module, path, mlist, platform, cmakelists, info, directory)
