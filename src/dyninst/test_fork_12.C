...
// $Id: test_fork_12.C,v 1.1 2008/10/30 19:21:32 legendre Exp $
/*
 * #Name: test7_8
...
class test_fork_12_Mutator : public DyninstMutator {
private:
  BPatch *bpatch;
...
extern "C" DLLEXPORT TestMutator *test_fork_12_factory() {
...
static void prepareTestCase8(procType proc_type, BPatch_thread *thread, forkWhen when)
{
   if(proc_type == Parent_p  &&  when == PreFork) {
       ...
       var7_8p = thread->getProcess()->malloc(*(parImage->findType("int")));
      ...
      const char *inFunction = "test_fork_12_func1";
	...
	logerror("%s[%d]:  WARNING  : found %d functions named %s.  Using the first.\n", 
      ...
      if(doError(&passedTest, !points7_8p || ((*points7_8p).size() == 0),
		 "  Unable to find entry point to \"test_fork_12_func1\".\n")) return;
      ...
      thread->getProcess()->insertSnippet(b_expr7_8p, *point7_8p, BPatch_callBefore);
   } else if(proc_type == Child_p  &&  when == PostFork) {
       BPatch_variableExpr *var7_8c = thread->getProcess()->getInheritedVariable(*var7_8p);
      thread->getProcess()->free(*var7_8c);
   }
}
...
test_results_t test_fork_12_Mutator::setup(ParameterDict &param) {
