...
class test_fork_14_Mutator : public DyninstMutator {
private:
  BPatch *bpatch;
...
extern "C" DLLEXPORT TestMutator *test_fork_14_factory() {
  return new test_fork_14_Mutator();
}
...
/* Run a oneTimeCode in both the parent and child and see if they both
   ...
   parent/child: test_fork_14_global1 initial value = 21
...
static void prepareTestCase6(procType proc_type, BPatch_thread *thread, forkWhen when)
{
   ...
   int addend = (proc_type == Parent_p) ? 5 : 9;
   BPatch_image *image = thread->getProcess()->getImage();

   BPatch_variableExpr *var =
      image->findVariable("test_fork_14_global1");
   if (doError(&passedTest, (var==NULL),
               "  Unable to locate variable test_fork_14_global1\n")) return;

   BPatch_arithExpr a_expr(BPatch_plus, *var, BPatch_constExpr(addend));
   BPatch_arithExpr b_expr(BPatch_assign, *var, a_expr);
...
{
   int value = (proc_type == Parent_p) ? 26 : 30;
   if (!verifyProcMemory(thread->getProcess(), "test_fork_14_global1",
                         value, proc_type))
      passedTest = false;
}
...
test_results_t test_fork_14_Mutator::executeTest() {
...
test_results_t test_fork_14_Mutator::setup(ParameterDict &param) {
